...
class KHR3HV(object):
    ...
    motion_bytes = ["\x07\x0c\xb8\x0b\x00\x00\xd6",     # 0
    ...
    cmd_ack = "\x04\xfe\x06\x08"
    cmd_getoptions = "\x0a\x00\x20\x00\x00\x00\x00\x00\x02\x2c"
    cmd_stopmotion = "\x13\x00\x02\x00\x00\x00\x21\x87\xfd\x03\x00\x00\x00\x00\x00\x00\x00\x00\xbd"
    cmd_restorestatus = "\x09\x00\x02\x00\x00\x00\x23\x87\xb5"
    cmd_querystatus = "\x0a\x00\x20\x00\x00\x00\x00\x00\x02\x2c"
    ...
    def play_motion(self, motion, timeout=None):
        ...
        self.device.write(self.cmd_stopmotion)
        ret = self.device.read(4)
        #print a.encode("hex")

        # write array for Play Motion
        self.device.write(self.motion_bytes[motion])
        ret = self.device.read(4)

        # write array for Restore memory
        self.device.write(self.cmd_restorestatus)
        ret = self.device.read(4)

        # new feature to test: Query for execution status
        while 1:
            self.device.write(self.cmd_querystatus)
